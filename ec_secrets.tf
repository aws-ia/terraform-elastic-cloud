locals {
  ...
  aws_access_key = sensitive(var.s3_client_access_key == "hashicorp/vault" ? "${data.vault_generic_secret.aws_creds.data["s3_client_access_key"]}" : var.s3_client_access_key)
  aws_secret_key = sensitive(var.s3_client_secret_key == "hashicorp/vault" ? "${data.vault_generic_secret.aws_creds.data["s3_client_secret_key"]}" : var.s3_client_secret_key)
  ess_api_key    = sensitive(var.apikey == "hashicorp/vault" ? "${data.vault_generic_secret.ess_creds.data["apikey"]}" : var.apikey)
}
...
# HashiCorp Vault – Reads AWS credentials if Vault server URL is provided
data "vault_generic_secret" "aws_creds" {
  path = var.vault_aws_path
}

# HashiCorp Vault – Reads ESS credentials if Vault server URL is provided
